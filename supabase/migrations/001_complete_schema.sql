-- ============================================================
-- GHOST PM — Complete Database Schema (Unified)
-- Run this ONCE in Supabase SQL Editor (Dashboard > SQL Editor > New query)
-- Includes: Base schema (teams, brainstorm, roadmaps, etc.)
--           + CLI extensions (commits, graph snapshots, AI advice)
-- ============================================================
...
-- ============================================================
-- TEAM MEMBERS (with CLI activity tracking columns)
-- ============================================================
CREATE TABLE team_members (
  ...
  distraction_score NUMERIC DEFAULT 0,
...
-- ============================================================
-- BRAINSTORM MESSAGES (also used for CLI terminal chat)
-- ============================================================
...
CREATE TABLE commits (
  ...
  milestone_id UUID REFERENCES roadmaps(id),
  scope_verdict JSONB DEFAULT '{}',
  committed_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_commits_team ON commits(team_id);
CREATE INDEX idx_commits_member ON commits(member_name);
CREATE INDEX idx_commits_time ON commits(committed_at DESC);

-- ============================================================
-- CLI EXTENSION: CODE GRAPH SNAPSHOTS
-- ============================================================
CREATE TABLE code_graph_snapshots (
  ...
  function_statuses JSONB DEFAULT '{}',
  god_nodes JSONB DEFAULT '[]',
  communities JSONB DEFAULT '[]',
  snapshot_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_graph_snapshots_team ON code_graph_snapshots(team_id);
CREATE INDEX idx_graph_snapshots_time ON code_graph_snapshots(snapshot_at DESC);

-- ============================================================
-- CLI EXTENSION: AI ADVICE LOG
-- ============================================================
CREATE TABLE ai_advice_log (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  team_id UUID NOT NULL REFERENCES teams(id) ON DELETE CASCADE,
  member_name TEXT NOT NULL,
  advice_type TEXT NOT NULL DEFAULT 'general',
  prompt_summary TEXT,
...
CREATE INDEX idx_ai_advice_team ON ai_advice_log(team_id);
...
-- Get or create CLI chat session
CREATE OR REPLACE FUNCTION get_or_create_cli_chat_session(p_team_id UUID)
RETURNS UUID AS $$
DECLARE
  session_id UUID;
BEGIN
  ...
  WHERE team_id = p_team_id AND anchor_text = '__cli_chat__' AND is_active = TRUE
  LIMIT 1;

  IF session_id IS NULL THEN
    INSERT INTO brainstorm_sessions (team_id, anchor_text, is_active)
    VALUES (p_team_id, '__cli_chat__', TRUE)
...
CREATE POLICY "members_update" ON team_members FOR UPDATE USING (
...
CREATE POLICY "shared_resources_all" ON shared_resources FOR ALL USING (
  team_id IN (SELECT team_id FROM team_members WHERE user_id = auth.uid())
);

-- COMMITS
ALTER TABLE commits ENABLE ROW LEVEL SECURITY;
CREATE POLICY "commits_select" ON commits FOR SELECT USING (is_team_member(team_id));
CREATE POLICY "commits_insert" ON commits FOR INSERT WITH CHECK (is_team_member(team_id));
CREATE POLICY "commits_update" ON commits FOR UPDATE USING (is_team_member(team_id));

-- CODE GRAPH SNAPSHOTS
ALTER TABLE code_graph_snapshots ENABLE ROW LEVEL SECURITY;
CREATE POLICY "snapshots_select" ON code_graph_snapshots FOR SELECT USING (is_team_member(team_id));
CREATE POLICY "snapshots_insert" ON code_graph_snapshots FOR INSERT WITH CHECK (is_team_member(team_id));

-- AI ADVICE LOG
ALTER TABLE ai_advice_log ENABLE ROW LEVEL SECURITY;
CREATE POLICY "advice_select" ON ai_advice_log FOR SELECT USING (is_team_member(team_id));
CREATE POLICY "advice_insert" ON ai_advice_log FOR INSERT WITH CHECK (is_team_member(team_id));
...
ALTER PUBLICATION supabase_realtime ADD TABLE ai_advice_log;
